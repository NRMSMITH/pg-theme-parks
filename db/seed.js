...
function createParks() {
  return db.query("DROP TABLE IF EXISTS parks;").then(() => {
    return db.query(
      "CREATE TABLE parks ( park_id SERIAL PRIMARY KEY, park_name VARCHAR(40) NOT NULL, year_opened INT NOT NULL, annual_attendance INT NOT NULL);"
    );
  });
}
...
function insertParks() {
  const sql = pg(
    ...
    parks.map(({ park_name, year_opened, annual_attendance }) => [
      park_name,
      year_opened,
      annual_attendance,
    ])
...
function createRides() {
  return db.query("DROP TABLE IF EXISTS rides").then(() => {
    return db.query(
      "CREATE TABLE rides ( ride_id SERIAL PRIMARY KEY, ride_name VARCHAR(20) NOT NULL, year_opened INT NOT NULL, votes INT DEFAULT 0, park_id INT REFERENCES parks(park_id) );"
    );
  });
}
